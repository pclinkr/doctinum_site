...
import SolutionStatementSection from '../components/sections/page-sections/SolutionStatementSection';
          ...
          <div className="space-y-6">
            <div>
              <p className="mb-3 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">
                {copy.patientPerception.label}
              </p>
              <ul className="space-y-2">
                {copy.patientPerception.items.map((item, index) => (
                  <li
                    key={index}
            ...
            <div>
              <p className="mb-3 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">
                {copy.clinicBenefits.label}
              </p>
              <ul className="space-y-2">
                {copy.clinicBenefits.items.map((item, index) => (
                  <li
                    key={index}
                    className="flex items-center gap-3 text-[15px] leading-[1.5] text-[var(--color-primary)]"
                  >
...
export default function RhinoplastyPage({ active, onNavigate }) {
  const { t } = useTranslation();
  const ctaSubtitleFull = getCtaSubtitleFull(t);
  const copy = t('pages.solutions.rhinoplasty', { returnObjects: true });
  ...
  return (
    ...
    >
      <SolutionHeroSection
        ...
        backButtonText="← Chirurgie & médecine esthétique"
        backButtonPage="surgery-aesthetic"
        nextButtonText="Liposuccion →"
        nextButtonPage="surgery-liposuction"
        backgroundImage="/assets/images/rhinoplastie.png"
      ...
      <TestimonialSection
        ...
        showVisual={true}
        visualImageUrl="/assets/images/rhinoplastie.png"
        showAuthor={false}
      />
