...
export default function UseCaseBlock({
  ...
  return (
    <article
      className={`rev mb-12 grid grid-cols-2 items-center gap-10 border-b border-[var(--border)] pb-12 last:mb-0 last:border-b-0 max-[809px]:flex max-[809px]:flex-col ${className}`}
    >
      <button
        type="button"
      ...
      <div
        className={`flex flex-col gap-3 ${isImageRight ? 'min-[810px]:order-1' : 'min-[810px]:order-2'}`.trim()}
      >
        <h3 className="text-[clamp(24px,3.1vw,36px)] font-[var(--w500)] leading-[1.08] tracking-[-0.05em] text-[var(--color-primary)]">
          {title}
        </h3>

        <div className="mt-1 flex flex-wrap gap-2">{renderButtons()}</div>
      </div>
    </article>
  );
}
