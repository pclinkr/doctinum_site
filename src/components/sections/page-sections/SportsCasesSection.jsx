...
function SportsCaseRow({ title, body, listItems = [], imagePosition = 'right', imageGradient, brand, onNavigate }) {
  ...
  return (
    <div className="rev mb-12 grid grid-cols-2 items-center gap-10 border-b border-[var(--border)] pb-12 last:mb-0 last:border-b-0 max-[809px]:flex max-[809px]:flex-col">
      ...
      <div className={`flex flex-col gap-4 ${isImageRight ? 'min-[810px]:order-1' : 'min-[810px]:order-2'}`.trim()}>
        <h3 className="text-[clamp(26px,3vw,38px)] font-[var(--w500)] leading-[1.08] tracking-[-0.05em]">{title}</h3>
        {body ? <p className="text-[14px] leading-[1.7] text-[var(--muted)]">{body}</p> : null}
        {listItems.length > 0 ? (
