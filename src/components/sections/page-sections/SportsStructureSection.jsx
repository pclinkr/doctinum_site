...
const ICONS = {
  ...
  visibility: <path d="M2 12s4-6 10-6 10 6 10 6-4 6-10 6-10-6-10-6zm10-3a3 3 0 1 0 0 6 3 3 0 0 0 0-6z" />
};
...
export default function SportsStructureSection() {
  ...
  const items = copy.items.map((item, index) => {
    const iconsByIndex = [ICONS.structure, ICONS.support, ICONS.indicators, ICONS.visibility];
  ...
  return (
    <section className="mx-auto w-full max-w-[1200px] px-16 pb-28 pt-8 text-center max-[1024px]:px-8 max-[768px]:px-5">
      <SectionHeading className="rev" title={<>{title}</>} style={{ marginBottom: '14px' }} />
      <p className="rev mx-auto mb-14 max-w-[760px] text-[16px] leading-[1.7] text-[var(--muted)]">{subtitle}</p>
      ...
      <div className="relative mx-auto max-w-[1080px]">
        ...
        {items.map((item, index) => (
          <article key={item.title} className={`rev ${index === 0 ? 'd1' : index === 1 ? 'd2' : index === 2 ? 'd3' : 'd4'} flex flex-col items-center text-center`}>
            ...
            <div className="w-full rounded-[18px] bg-transparent px-4 py-5">
              <p className="text-[17px] leading-[1.5] text-[var(--color-primary)]">{item.title}</p>
            </div>
          </article>
        ))}
        </div>
      ...
      <p className="rev mt-14 text-[16px] leading-[1.7] text-[var(--muted)]">{closing}</p>
