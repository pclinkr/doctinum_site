...
export default function HomeSecurityFrameSection({ onNavigate }) {
  ...
  return (
    <section className="mx-auto w-full max-w-[1200px] px-16 py-24 max-[1024px]:px-8 max-[768px]:px-5" id="homeSecurityFrame">
      <div className="rev mb-10 text-center">
        ...
        <h2 className="mx-auto max-w-[900px] text-[var(--color-primary)]">{t('sections.homeSecurityFrame.title')}</h2>
      ...
      <div className="rev overflow-hidden border-t border-[var(--border)]">
        <table className="w-full border-collapse">
          <tbody>
            {rows.map((row, index) => (
              <tr key={row.title}>
                <td className={`w-[34%] px-2 py-5 align-top text-[18px] font-[var(--w500)] leading-[1.2] tracking-[-0.03em] text-[var(--color-primary)] ${index < rows.length - 1 ? 'border-b border-[var(--border)]' : ''}`.trim()}>
                  {row.title}
                </td>
                <td className={`px-2 py-5 align-top text-[15px] leading-[1.7] text-[var(--muted)] ${index < rows.length - 1 ? 'border-b border-[var(--border)]' : ''}`.trim()}>
                  {row.body}
                </td>
              </tr>
            ))}
