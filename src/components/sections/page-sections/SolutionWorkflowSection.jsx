...
const STEP_TONES = [
  {
    bg: 'bg-[linear-gradient(160deg,#ffffff_0%,#f7f9ff_100%)]',
    border: 'border-[#dde4f2]',
  },
  {
    bg: 'bg-[linear-gradient(160deg,#ffffff_0%,#f5faf8_100%)]',
    border: 'border-[#dbece4]',
  },
  {
    bg: 'bg-[linear-gradient(160deg,#ffffff_0%,#f8f6ff_100%)]',
    border: 'border-[#e3ddf4]',
  },
  {
    bg: 'bg-[linear-gradient(160deg,#ffffff_0%,#f6fbff_100%)]',
    border: 'border-[#dce9f5]',
  },
  {
    bg: 'bg-[linear-gradient(160deg,#ffffff_0%,#fbf8f4_100%)]',
    border: 'border-[#ebe1d7]',
  },
];

export default function SolutionWorkflowSection() {
  const { t } = useTranslation();
  const workflowSteps = t('sections.homeOurSolution.workflow.steps', {
    returnObjects: true,
  });
  ...
  return (
    <div className="rev rounded-[24px] border border-[var(--border)] bg-[linear-gradient(160deg,var(--bg-white)_0%,var(--bg-surface)_100%)] p-8 max-[809px]:p-6">
      <div className="mb-7 flex items-end justify-between gap-5 max-[809px]:flex-col max-[809px]:items-start">
        <div>
          <p className="mb-2 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">
            {t('sections.homeOurSolution.workflow.eyebrow')}
          </p>
      ...
      <div className="grid grid-cols-[1.15fr_.85fr] gap-8 max-[980px]:grid-cols-1">
        <div className="flex flex-col gap-3">
          {workflowSteps.map((stepItem, index) => (
            <div
              key={stepItem.title}
              className={`rev grid grid-cols-[38px_1fr] gap-4 rounded-[16px] border p-5 ${STEP_TONES[index]?.bg || 'bg-[var(--surface)]'} ${STEP_TONES[index]?.border || 'border-[var(--border)]'} ${index === 0 ? 'd1' : index === 1 ? 'd2' : index === 2 ? 'd3' : index === 3 ? 'd4' : ''}`.trim()}
            >
              ...
              <div>
                <h4 className="mb-2 text-[17px] font-[var(--w500)] tracking-[-0.02em] text-[var(--color-primary)]">
                  {stepItem.title}
                </h4>
                <p className="text-[14px] leading-[1.66] text-[var(--muted)]">
                  {stepItem.body}
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-rows-[auto_1fr] gap-4">
          <div className="rev rounded-[16px] border border-[var(--border)] bg-[var(--surface)] p-5">
            <p className="mb-2 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">
              {t('sections.homeOurSolution.visual.alertTitle')}
            </p>
          ...
          <div className="rev d2 rounded-[16px] border border-[var(--border)] bg-[var(--surface)] p-5">
            <p className="mb-3 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">
              {t('sections.homeOurSolution.visual.chartTitle')}
            </p>
            <div className="space-y-4">
              <div className="rounded-[12px] border border-[var(--ink-10)] bg-[var(--bg-page)] p-3">
                <p className="mb-2 text-[11px] text-[var(--muted)]">
                  {t('sections.homeOurSolution.visual.chartLineLabel')}
                </p>
                <svg viewBox="0 0 240 70" className="h-[72px] w-full">
                  <path
                    d="M6 56 C30 46, 40 48, 58 40 C76 33, 90 36, 112 28 C134 20, 150 26, 174 18 C194 12, 210 16, 234 8"
                    fill="none"
                    stroke="rgba(58,92,168,.45)"
                    strokeWidth="2.2"
                  />
                  <path
                    d="M6 60 C32 51, 56 54, 78 46 C102 38, 122 43, 146 35 C170 28, 196 30, 234 22"
                    fill="none"
                    stroke="rgba(38,146,108,.5)"
                    strokeWidth="2"
                  />
                </svg>
              </div>
              ...
              <div className="rounded-[12px] border border-[var(--ink-10)] bg-[var(--bg-page)] p-3">
                <p className="mb-2 text-[11px] text-[var(--muted)]">
                  {t('sections.homeOurSolution.visual.chartBarsLabel')}
                </p>
              ...
              <div className="flex items-center gap-2">
                <span className="rounded-full border border-[rgba(58,92,168,.25)] bg-[rgba(58,92,168,.1)] px-2 py-1 text-[11px] text-[rgba(43,66,118,.9)]">
                  {t('sections.homeOurSolution.visual.badges.lowAlert')}
                </span>
                <span className="rounded-full border border-[rgba(38,146,108,.25)] bg-[rgba(38,146,108,.1)] px-2 py-1 text-[11px] text-[rgba(23,106,77,.9)]">
                  {t('sections.homeOurSolution.visual.badges.stable')}
                </span>
                <span className="rounded-full border border-[rgba(164,126,70,.22)] bg-[rgba(164,126,70,.1)] px-2 py-1 text-[11px] text-[rgba(126,93,45,.9)]">
                  {t('sections.homeOurSolution.visual.badges.monitoring')}
                </span>
              </div>
            </div>
          </div>
