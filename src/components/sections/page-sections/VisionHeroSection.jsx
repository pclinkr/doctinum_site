  ...
  return (
    <section className="relative overflow-hidden pb-20 pt-[calc(var(--nav-h)+56px)]">
      ...
      <Container className="relative z-10 flex min-h-[60vh] flex-col justify-center">
        <p className="mb-8 text-[12px] font-[var(--w500)] uppercase tracking-[0.16em] text-color-primary opacity-0 animate-[fadeUp_0.8s_ease_0.1s_forwards]">
          {eyebrow}
        </p>

        <h1 className="mb-6 max-w-[16ch] font-serif text-[clamp(2.4rem,4.6vw,3.8rem)] font-[600] leading-[1.1] tracking-[-0.02em] text-[var(--color-text)] opacity-0 animate-fadeUp">
          {title}
        </h1>
          ...
          <div className="flex gap-16 pt-12 opacity-0 animate-[fadeUp_0.9s_ease_0.55s_forwards]">
            {stats.map((stat, index) => (
              <div key={index}>
                ...
                <div className="mt-3 text-[14px] font-[var(--w500)] leading-[1.5] tracking-[-0.01em] text-ink-68">
                  {stat.label}
                </div>
              </div>
            ))}
