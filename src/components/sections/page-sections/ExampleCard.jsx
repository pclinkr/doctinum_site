...
export default function ExampleCard({
  ...
  return (
    ...
    >
      <div
        className={`relative ${aspectRatio} overflow-hidden rounded-[16px] border border-[var(--border)]`}
      >
        {cardItem.imageUrl && (
          <>
            <div
            ...
            {showOverlay && (
              <div
                ...
                style={{
                  background: cardItem.overlayGradient || overlayGradient,
                }}
              />
            )}
          </>
      ...
      <div className="relative mt-3 rounded-[16px] bg-[var(--surface)] px-4 pb-4 pt-4 pr-12">
        <h3 className="text-[clamp(18px,1.45vw,24px)] leading-[1.12] tracking-[-0.02em] text-[var(--color-primary)]">
          {cardItem.title}
        </h3>
        {cardItem.description && (
          <p className="mt-2 text-[14px] leading-[1.58] text-[var(--ink-50)]">
            {cardItem.description}
          </p>
        )}
        {showButton && (
