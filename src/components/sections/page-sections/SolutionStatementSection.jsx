    ...
    <div className={`rev mb-10 ${className}`}>
      <div className={`grid grid-cols-[1.1fr_.9fr] gap-8 ${listWidth}`}>
        <div className="flex flex-col gap-4">
          <h3
            className={`text-[clamp(24px,3.2vw,36px)] font-[var(--w500)] leading-[1.08] tracking-[-0.045em] ${titleColor}`}
          >
            {statementTitle && (
              <>
          ...
          {statementBody && (
            <p
              className={`max-w-[65ch] ${textSize} leading-[1.72] text-[var(--muted)]`}
            >
              {statementBody}
            </p>
          )}
        ...
        <div className="relative pt-2 pl-5 max-[980px]:pl-0">
          ...
          />
          <div
            className="pointer-events-none absolute left-0 top-0 h-full w-px max-[980px]:hidden"
          ...
          <div className="flex flex-col">
            {impactItems.map((itemText, index) => (
              <div
                key={index}
                className={`rev relative grid grid-cols-[60px_1fr] items-start gap-3 py-4 ${index < impactItems.length - 1 ? borderColor : ''} ${index === 0 ? 'd1' : index === 1 ? 'd2' : 'd3'}`.trim()}
              >
                <span
                  className={`pt-[1px] ${numberSize} font-[var(--w500)] leading-none tracking-[-0.04em] ${numberColor}`}
                >
                  {String(index + 1).padStart(2, '0')}
                </span>
                <p
                  className={`max-w-[36ch] pt-[5px] font-[var(--w500)] leading-[1.14] tracking-[-0.028em] ${textColor}`}
                >
                  {itemText}
                </p>
              </div>
