    ...
    <section className="mx-auto w-full max-w-[1200px] px-16 pb-24 pt-0 max-[1024px]:px-8 max-[768px]:px-5">
      <div className="grid grid-cols-2 gap-5 max-[809px]:grid-cols-1">
        {cards.map((caseItem) => (
          <Card
            key={caseItem.name}
            className={`group rev cursor-pointer overflow-hidden rounded-[var(--r-md)] bg-[var(--surface)] transition-transform duration-300 ease-spring transition-shadow duration-300 ease-out hover:translate-y-px hover:shadow-[0_16px_48px_var(--ink-10)] ${caseItem.delayClass}`.trim()}
          ...
          >
            ...
            >
              ...
              >
                <span
                  className="text-[22px] font-[var(--w500)] tracking-[-0.04em] text-white shadow-[0_2px_12px_var(--ink-30)]"
                  style={
                    caseItem.serif
                      ? { fontFamily: 'var(--serif)', fontStyle: 'italic' }
                      : undefined
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <span className="text-[15px] font-[var(--w500)] tracking-[var(--track)]">
                {caseItem.name}
              </span>
