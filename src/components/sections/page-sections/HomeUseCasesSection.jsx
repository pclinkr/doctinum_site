...
export default function HomeUseCasesSection({ onNavigate }) {
  ...
  return (
    <section className="mx-auto w-full max-w-[1200px] px-16 py-24 max-[1024px]:px-8 max-[768px]:px-5" id="homeUseCases">
      <div className="rev mb-11">
        <p className="mb-3 text-[11px] font-[var(--w500)] uppercase tracking-[0.07em] text-[var(--muted)]">{t('sections.homeUseCases.eyebrow')}</p>
        <h2 className="max-w-[860px] text-[var(--color-primary)]">{t('sections.homeUseCases.title')}</h2>
      </div>
      ...
      {items.map((item, index) => {
        ...
        const mediaConfig = index === 0 
          ...
          : {
              type: 'gradient',
              gradient: PLACEHOLDER_GRADIENTS[index] || PLACEHOLDER_GRADIENTS[0]
            };
