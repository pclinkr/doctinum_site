...
export default function NavBar({
  ...
  const openMegaMenu = useMemo(
    () =>
      navItems.find((item) => item.id === openMegaId && item.megaMenu)
        ?.megaMenu,
    [openMegaId, navItems]
  );
  ...
  return (
    <nav
      ...
      id="mainNav"
      style={{
        '--nav-scroll-progress': Math.min(1, Math.max(0, blurProgress)),
      }}
    >
      {isDrawerMode ? (
        <div className="relative z-[3] flex h-[var(--nav-h)] w-full max-w-none items-center justify-between px-5">
          <div
            className="flex cursor-pointer items-center gap-[6px] shrink-0"
          ...
          <div className="ml-auto flex h-full items-center gap-3">
            ...
            <button
              className="relative h-[44px] w-[44px] shrink-0 items-center justify-center flex"
              id="navBurger"
            ...
            >
              <span
                className={`absolute right-[12px] h-[2px] rounded-[2px] bg-[var(--color-primary)] transition-all duration-150 ease-out ${isMenuOpen ? 'top-[calc(50%-1px)] w-[20px] rotate-45' : 'top-[calc(40%-1px)] w-[14px]'}`.trim()}
              />
              <span
                className={`absolute right-[12px] h-[2px] rounded-[2px] bg-[var(--color-primary)] transition-all duration-150 ease-out ${isMenuOpen ? 'top-[calc(50%-1px)] w-[20px] -rotate-45' : 'top-[calc(58%-1px)] w-[20px]'}`.trim()}
              />
            </button>
          </div>
        </div>
        ...
        <div className="relative z-[3] mx-auto flex h-[var(--nav-h)] w-full max-w-[1280px] items-center px-6 min-[1100px]:px-10">
          <div
            className="flex cursor-pointer items-center gap-[6px] shrink-0 pr-3"
            onClick={() => handleNavigate('home')}
          >
            <BrandWordmark
              className="block h-6 w-auto"
          ...
          <div
            className="flex h-full min-w-0 flex-1 items-center justify-center gap-3 px-2 min-[1000px]:gap-4 min-[1240px]:gap-5 min-[1000px]:px-4"
            id="navLinks"
          >
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.megaMenu && (
                    <span
                      className={`text-[10px] leading-none translate-y-px transition-all duration-150 ease-out ${isOpen ? 'rotate-180 opacity-100' : 'opacity-[0.62]'}`.trim()}
                    >
                      ▾
                    </span>
                  )}
      ...
      <div
        className={`${isDrawerMode ? 'flex' : 'hidden'} fixed inset-x-0 top-[var(--nav-h)] bottom-0 z-[55] w-full max-w-none flex-col border-t border-[var(--border)] bg-[var(--bg-overlay-95)] px-5 shadow-[0_18px_40px_var(--ink-16)] backdrop-blur-[12px] transition-all duration-300 ease-out ${isMenuOpen ? 'overflow-hidden opacity-100' : 'pointer-events-none overflow-hidden opacity-0'}`.trim()}
        id="navDrawer"
      >
        <div className="sticky top-0 z-[2] -mx-5 border-b border-[var(--ink-08)] bg-[var(--bg-overlay-95)] px-5 py-3 backdrop-blur-[10px]">
          <p className="text-[12px] font-[var(--w500)] uppercase tracking-[0.08em] text-[var(--muted)]">
            {t('nav.mobileMenu')}
          </p>
        </div>
          ...
          <div className="flex flex-col">
            {mobileSections.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  className="flex flex-col border-b border-[var(--ink-08)] py-1 last:border-b-0"
                >
                  <button
                  ...
                  >
                    ...
                    {hasNestedContent && (
                      <span
                        className={`text-[11px] leading-none transition-all duration-150 ease-out ${isOpen ? 'rotate-180 opacity-100' : 'opacity-[0.62]'}`.trim()}
                      >
                        ▾
                      </span>
                    )}
                  ...
                  {hasNestedContent && (
                    <div
                      className={`grid transition-all duration-300 ease-out ${isOpen ? 'grid-rows-[1fr] pb-2' : 'grid-rows-[0fr]'}`.trim()}
                    >
                        ...
                        <div className="ml-3 border-l border-[var(--ink-08)] pl-3">
                          {(section.groups || []).map((group, groupIndex) => (
                            <div
                              ...
                              style={
                                isOpen
                                  ? { transitionDelay: `${groupIndex * 45}ms` }
                                  : undefined
                              }
                            >
                              <p className="mb-1 text-[11px] font-[var(--w500)] tracking-[var(--track)] text-[var(--muted)]">
                                {group.title}
                              </p>
                              {group.links.map((link, linkIndex) => {
                                ...
                                return (
                                  <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                      link.page && handleNavigate(link.page)
                                    }
      ...
      <div
        className={`absolute left-1/2 top-full z-[25] w-screen -translate-x-1/2 pt-0 transition-opacity duration-200 ease-out ${!isDrawerMode && openMegaMenu ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`.trim()}
        onMouseLeave={() => setOpenMegaId('')}
      >
        <NavMegaMenu
          menu={openMegaMenu}
