...
export default function VoiceCallOrb({ mode = 'listening', isActive = false }) {
  ...
  return (
    <div
      className={`relative flex h-full w-full items-center justify-center overflow-hidden rounded-full transition-all duration-240 ease-out ${normalizedMode === 'speaking' ? 'scale-[1.01]' : 'scale-[0.996]'}`.trim()}
      aria-hidden="true"
    >
      <Orb
